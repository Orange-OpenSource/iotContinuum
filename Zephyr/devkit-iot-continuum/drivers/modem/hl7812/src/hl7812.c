...
#define LOG_MODULE_NAME modem_hl7812
 ...
 */
/* #define HL7812_ENABLE_VERBOSE_MODEM_RECV_HEXDUMP 1 */

#define HL7812_LOG_UNHANDLED_RX_MSGS 1

/* Uncomment the #define(s) below to enable extra debugging */
/* #define HL7812_RX_LOCK_LOG 1 */
/* #define HL7812_TX_LOCK_LOG 1 */
#define HL7812_IO_LOG                1

#define HL7812_RX_LOCK_DBG_LOG(fmt, ...) \
   do                                    \
   {                                     \
      if(IS_ENABLED(HL7812_RX_LOCK_LOG)) \
      {                                  \
...
#define HL7812_TX_LOCK_DBG_LOG(fmt, ...) \
   do                                    \
   {                                     \
      if(IS_ENABLED(HL7812_TX_LOCK_LOG)) \
      {                                  \
         LOG_DBG(fmt, ##__VA_ARGS__);    \
      }                                  \
   }                                     \
...
#define HL7812_IO_DBG_LOG(fmt, ...)   \
   do                                 \
   {                                  \
      if(IS_ENABLED(HL7812_IO_LOG))   \
      {                               \
         LOG_DBG(fmt, ##__VA_ARGS__); \
      }                               \
   }                                  \
...
#if((LOG_LEVEL == LOG_LEVEL_DBG) && defined(CONFIG_MODEM_HL7812_LOW_POWER_MODE))
...
enum udp_notif
{
   HL7812_UDP_NET_ERR  = 0,
   HL7812_UDP_NO_SOCKS = 1,
   HL7812_UDP_MEM      = 2,
   HL7812_UDP_DNS      = 3,
   HL7812_UDP_CONN     = 5,
   HL7812_UDP_ERR      = 6,
   HL7812_UDP_DATA_SND = 8, /* this matches TCP_DATA_SND */
   HL7812_UDP_ID       = 9,
   HL7812_UDP_RUNNING  = 10,
   HL7812_UDP_ALL_USED = 11
};
...
#define MDM_SN_RESPONSE_LENGTH         (MDM_HL7812_SERIAL_NUMBER_SIZE + 7)
...
#define MDM_BAND_BITMAP_STR_LENGTH_MAX (MDM_TOP_BAND_SIZE + MDM_MIDDLE_BAND_SIZE + MDM_BOTTOM_BAND_SIZE)
...
#define MDM_WAIT_FOR_HTTP_GET_TIME     K_SECONDS(5)
...
#if defined(CONFIG_MODEM_HL7812_ADDRESS_FAMILY_IPV4V6)
   #define MODEM_HL7812_ADDRESS_FAMILY "IPV4V6"
#elif defined(CONFIG_MODEM_HL7812_ADDRESS_FAMILY_IPV4)
   #define MODEM_HL7812_ADDRESS_FAMILY ADDRESS_FAMILY_IPV4
#else
   #define MODEM_HL7812_ADDRESS_FAMILY "IPV6"
#endif
#define MDM_HL7812_SOCKET_AF_IPV4 0
#define MDM_HL7812_SOCKET_AF_IPV6 1
...
#define NEW_RAT_CMD_MIN_VERSION   "HL7812.4.5.4.0"
#define HL7812_VERSION_FORMAT     "HL7812.%zu.%zu.%zu.%zu"

#define MAX_PROFILE_LINE_LENGTH   MAX(sizeof(PROFILE_LINE_1), sizeof(PROFILE_LINE_2))

#define IPV6_ADDR_FORMAT          "####:####:####:####:####:####:####:####"
#define HL7812_IPV6_ADDR_LEN      sizeof("a01.a02.a03.a04.a05.a06.a07.a08.a09.a10.a11.a12.a13.a14.a15.a16")

#define MDM_ADDR_FAM_MAX_LEN      sizeof("IPV4V6")

/* The ? can be a + or - */
static char const TIME_STRING_FORMAT[] = "\"yy/MM/dd,hh:mm:ss?zz\"";
...
NET_BUF_POOL_DEFINE(mdm_recv_pool,
                    CONFIG_MODEM_HL7812_RECV_BUF_CNT,
                    CONFIG_MODEM_HL7812_RECV_BUF_SIZE,
                    0,
                    NULL);

static uint8_t mdm_recv_buf[MDM_MAX_DATA_LENGTH];

static K_SEM_DEFINE(hl7812_RX_lock_sem, 1, 1);
static K_SEM_DEFINE(hl7812_TX_lock_sem, 1, 1);
static K_MUTEX_DEFINE(cb_lock);

/* RX thread structures */
K_THREAD_STACK_DEFINE(hl7812_rx_stack, CONFIG_MODEM_HL7812_RX_STACK_SIZE);
struct k_thread hl7812_rx_thread;
#define RX_THREAD_PRIORITY K_PRIO_COOP(7)

/* RX thread work queue */
K_THREAD_STACK_DEFINE(hl7812_workq_stack, CONFIG_MODEM_HL7812_RX_WORKQ_STACK_SIZE);
static struct k_work_q hl7812_workq;
#define WORKQ_PRIORITY K_PRIO_COOP(7)

static char const EOF_PATTERN[]    = "--EOF--Pattern--";
static char const CONNECT_STRING[] = "CONNECT";
static char const OK_STRING[]      = "OK";

struct hl7812_socket
{
...
struct hl7812_iface_ctx
{
   ...
   char            dns_v6_string[HL7812_IPV6_ADDR_LEN];
#endif
   bool     restarting;
   bool     initialized;
   bool     wait_for_KSUP;
   ...
   /* socket data */
   struct hl7812_socket sockets[MDM_MAX_SOCKETS];
   int                  last_socket_id;
   int                  last_error;
...
#ifdef CONFIG_MODEM_HL7812_FW_UPDATE
   /* firmware update */
   enum mdm_hl7812_fota_state fw_update_state;
   ...
   char                          mdm_revision[MDM_HL7812_REVISION_MAX_SIZE];
   char                          mdm_imei[MDM_HL7812_IMEI_SIZE];
   char                          mdm_sn[MDM_HL7812_SERIAL_NUMBER_SIZE];
   char                          mdm_network_status[MDM_NETWORK_STATUS_LENGTH];
   char                          mdm_iccid[MDM_HL7812_ICCID_SIZE];
   enum mdm_hl7812_startup_state mdm_startup_state;
   enum mdm_hl7812_radio_mode    mdm_rat;
   char                          mdm_active_bands_string[MDM_HL7812_LTE_BAND_STR_SIZE];
   char                          mdm_bands_string[MDM_HL7812_LTE_BAND_STR_SIZE];
   char                          mdm_imsi[MDM_HL7812_IMSI_MAX_STR_SIZE];
   int                           mdm_rssi;
   ...
   int32_t                       mdm_sinr;
   bool                          mdm_echo_is_on;
   struct mdm_hl7812_apn         mdm_apn;
   bool                          mdm_startup_reporting_on;
   int                           device_services_ind;
   bool                          new_rat_cmd_support;
   uint8_t                       operator_index;
   enum mdm_hl7812_functionality functionality;
   char                          mdm_pdp_addr_fam[MDM_ADDR_FAM_MAX_LEN];

   /* modem state */
   bool                          allow_sleep;
   bool                          uart_on;
   enum mdm_hl7812_sleep         desired_sleep_level;
   enum mdm_hl7812_sleep         sleep_state;
   enum hl7812_lpm               low_power_mode;
   enum mdm_hl7812_network_state network_state;
...
static sys_slist_t hl7812_event_callback_list = SYS_SLIST_STATIC_INIT(&hl7812_event_callback_list);

static const struct hl7812_config hl7812_cfg  = {
         .gpio =
                {
...
static struct hl7812_iface_ctx ictx;

static size_t                  hl7812_read_rx(struct net_buf **buf);
static char                   *get_network_state_string(enum mdm_hl7812_network_state state);
static char                   *get_startup_state_string(enum mdm_hl7812_startup_state state);
static char                   *get_sleep_state_string(enum mdm_hl7812_sleep state);
static void                    set_network_state(enum mdm_hl7812_network_state state);
static void                    set_startup_state(enum mdm_hl7812_startup_state state);
static void                    set_sleep_state(enum mdm_hl7812_sleep state);
...
#ifdef CONFIG_MODEM_HL7812_LOW_POWER_MODE
static void mark_sockets_for_reconfig(void);
#endif
static void hl7812_build_mac(struct hl7812_iface_ctx *ictx);
...
#ifdef CONFIG_MODEM_HL7812_FW_UPDATE
static char *get_fota_state_string(enum mdm_hl7812_fota_state state);
static void  set_fota_state(enum mdm_hl7812_fota_state state);
...
static int  read_pin(int default_state, const struct gpio_dt_spec *spec)
{
   ...
   if(state < 0)
   {
      LOG_ERR("Unable to read port: %s pin: %d status: %d", spec->port->name, spec->pin, state);
      state = default_state;
   }
...
{
   ictx.vgpio_state = read_pin(0, &hl7812_cfg.gpio[MDM_VGPIO]);

   ictx.gpio6_state = read_pin(0, &hl7812_cfg.gpio[MDM_GPIO6]);

   ictx.cts_state   = read_pin(1, &hl7812_cfg.gpio[MDM_UART_CTS]);
...
/**
 * The definition of awake is that the HL7812
 * is ready to receive AT commands successfully
 */
static void check_hl7812_awake(void)
{
#ifdef CONFIG_MODEM_HL7812_LOW_POWER_MODE
   bool is_cmd_rdy = is_cmd_ready();

   if(is_cmd_rdy && (ictx.sleep_state != HL7812_SLEEP_AWAKE) && !ictx.allow_sleep && !ictx.wait_for_KSUP)
   {
      PRINT_AWAKE_MSG;
      set_sleep_state(HL7812_SLEEP_AWAKE);
      k_sem_give(&ictx.mdm_awake);
   }
   else if(!is_cmd_rdy && ictx.sleep_state == HL7812_SLEEP_AWAKE && ictx.allow_sleep)
   {
      PRINT_NOT_AWAKE_MSG;

      if(ictx.desired_sleep_level == HL7812_SLEEP_HIBERNATE ||
         ictx.desired_sleep_level == HL7812_SLEEP_LITE_HIBERNATE)
      {
      ...
      else if(ictx.desired_sleep_level == HL7812_SLEEP_SLEEP)
      {
         set_sleep_state(HL7812_SLEEP_SLEEP);
      }
   }
#endif
}

static int hl7812_RX_lock(void)
{
   HL7812_RX_LOCK_DBG_LOG("Locking RX [%p]...", k_current_get());
   int rc = k_sem_take(&hl7812_RX_lock_sem, K_FOREVER);

   if(rc != 0)
   {
      LOG_ERR("Unable to lock hl7812 (%d)", rc);
   }
   else
   {
      HL7812_RX_LOCK_DBG_LOG("Locked RX [%p]", k_current_get());
   }

   return rc;
}

static void hl7812_RX_unlock(void)
{
   HL7812_RX_LOCK_DBG_LOG("UNLocking RX [%p]...", k_current_get());
   k_sem_give(&hl7812_RX_lock_sem);
   HL7812_RX_LOCK_DBG_LOG("UNLocked RX [%p]", k_current_get());
}

static bool hl7812_RX_locked(void)
{
   if(k_sem_count_get(&hl7812_RX_lock_sem) == 0)
   {
      return true;
   }
   else
...
static int hl7812_TX_lock(void)
{
   HL7812_TX_LOCK_DBG_LOG("Locking TX [%p]...", k_current_get());
   int rc = k_sem_take(&hl7812_TX_lock_sem, K_FOREVER);
   ...
   {
      HL7812_TX_LOCK_DBG_LOG("Locked TX [%p]", k_current_get());
   }

   return rc;
}

static void hl7812_TX_unlock(void)
{
   HL7812_TX_LOCK_DBG_LOG("UNLocking TX [%p]...", k_current_get());
   k_sem_give(&hl7812_TX_lock_sem);
   HL7812_TX_LOCK_DBG_LOG("UNLocked TX [%p]", k_current_get());
}

static bool hl7812_TX_locked(void)
{
   if(k_sem_count_get(&hl7812_TX_lock_sem) == 0)
   {
      return true;
   }
   else
...
static void hl7812_lock(void)
{
   hl7812_TX_lock();
   hl7812_RX_lock();
}

static void hl7812_unlock(void)
{
   hl7812_RX_unlock();
   hl7812_TX_unlock();
}

static struct hl7812_socket *socket_get(void)
{
   int                   i;
   struct hl7812_socket *sock = NULL;
...
static struct hl7812_socket *socket_from_id(int socket_id)
{
   int                   i;
   struct hl7812_socket *sock = NULL;
...
static void socket_put(struct hl7812_socket *sock)
{
   if(!sock)
   {
      return;
...
static struct hl7812_socket *socket_http_create(void)
{
   struct hl7812_socket *sock = NULL;
   ...
   sock->reconfig  = false;
   sock->created   = false;
   sock->socket_id = MDM_HTTP_SOCKET_ID;

   return sock;
}

char *hl7812_sprint_ip_addr(const struct sockaddr *addr)
{
...
static void modem_assert_wake(bool assert)
{
   ...
   if(assert)
   {
      HL7812_IO_DBG_LOG("MDM_WAKE_PIN -> ASSERTED");
      state = 1;
   }
   else
   {
      HL7812_IO_DBG_LOG("MDM_WAKE_PIN -> NOT_ASSERTED");
      state = 0;
   }

   gpio_pin_set_dt(&hl7812_cfg.gpio[MDM_WAKE], state);
...
static void modem_assert_pwr_on(bool assert)
{
   if(assert)
   {
      HL7812_IO_DBG_LOG("MDM_PWR_ON -> ASSERTED");
      gpio_pin_set_dt(&hl7812_cfg.gpio[MDM_PWR_ON], 1);
   }
   else
   {
      HL7812_IO_DBG_LOG("MDM_PWR_ON -> NOT_ASSERTED");
      gpio_pin_set_dt(&hl7812_cfg.gpio[MDM_PWR_ON], 0);
   }
}
...
static void modem_assert_fast_shutd(bool assert)
{
   if(assert)
   {
      HL7812_IO_DBG_LOG("MDM_FAST_SHUTD -> ASSERTED");
      gpio_pin_set_dt(&hl7812_cfg.gpio[MDM_FAST_SHUTD], 1);
   }
   else
   {
      HL7812_IO_DBG_LOG("MDM_FAST_SHUTD -> NOT_ASSERTED");
      gpio_pin_set_dt(&hl7812_cfg.gpio[MDM_FAST_SHUTD], 0);
   }
}
...
static void allow_sleep(bool allow)
{
   ...
   if(allow)
   {
      k_work_reschedule_for_queue(&hl7812_workq,
                                  &ictx.allow_sleep_work,
                                  K_MSEC(CONFIG_MODEM_HL7812_ALLOW_SLEEP_DELAY_MS));
   }
   else
   {
      LOG_DBG("Keep awake");
...
static void event_handler(enum mdm_hl7812_event event, void *event_data)
{
   sys_snode_t                      *node;
   struct mdm_hl7812_callback_agent *agent;

   k_mutex_lock(&cb_lock, K_FOREVER);
   SYS_SLIST_FOR_EACH_NODE(&hl7812_event_callback_list, node)
   {
      agent = CONTAINER_OF(node, struct mdm_hl7812_callback_agent, node);
...
{
   if(CONFIG_MODEM_HL7812_RSSI_RATE_SECONDS == 0)
   {
      rssi_query();
   }
...
void mdm_hl7812_wakeup(bool wakeup)
{
   allow_sleep(!wakeup);
}

/* Send an AT command with a series of response handlers */
static int send_at_cmd(struct hl7812_socket *sock,
                       uint8_t const        *data,
                       k_timeout_t           timeout,
                       int                   retries,
                       bool                  no_id_resp)
...
static int wakeup_hl7812(void)
{
    ...
    */
   if(ictx.desired_sleep_level == HL7812_SLEEP_SLEEP)
   {
...
int32_t mdm_hl7812_update_apn(char *access_point_name)
{
   ...
   if(ret >= 0)
   {
       ...
       */
      k_work_reschedule_for_queue(&hl7812_workq, &ictx.mdm_reset_work, K_NO_WAIT);
   }
   return ret;
}
...
int32_t mdm_hl7812_update_rat(enum mdm_hl7812_radio_mode value)
{
   ...
   else if(!mdm_hl7812_valid_rat(value))
   {
      return ret;
   }
...
int32_t mdm_hl7812_set_gps_rate(uint32_t rate)
{
   ...
   else
   {
      ...
      if(IS_ENABLED(CONFIG_MODEM_HL7812_USE_GLONASS))
      {
...
#endif /* CONFIG_MODEM_HL7812_GPS */

#ifdef CONFIG_MODEM_HL7812_POLTE
...
{
   int  ret                                     = -1;
   char buf[sizeof(MDM_HL7812_SET_POLTE_USER_AND_PASSWORD_FMT_STR) + MDM_HL7812_MAX_POLTE_USER_ID_SIZE +
            MDM_HL7812_MAX_POLTE_PASSWORD_SIZE] = {0};
   ...
   snprintk(buf, sizeof(buf), MDM_HL7812_SET_POLTE_USER_AND_PASSWORD_FMT_STR, user, password);
   ret = send_at_cmd(NULL, buf, MDM_CMD_SEND_TIMEOUT, MDM_DEFAULT_AT_CMD_RETRIES, false);
...
#endif /* CONFIG_MODEM_HL7812_POLTE */
...
#endif
   event_handler(HL7812_EVENT_RSSI, &ictx.mdm_rssi);
   event_handler(HL7812_EVENT_SINR, &ictx.mdm_sinr);
   event_handler(HL7812_EVENT_APN_UPDATE, &ictx.mdm_apn);
   event_handler(HL7812_EVENT_RAT, &ictx.mdm_rat);
   event_handler(HL7812_EVENT_BANDS, ictx.mdm_bands_string);
   event_handler(HL7812_EVENT_ACTIVE_BANDS, ictx.mdm_active_bands_string);
   event_handler(HL7812_EVENT_REVISION, ictx.mdm_revision);
   hl7812_unlock();
}
...
#ifdef CONFIG_LOG
   new_log_level =
           log_filter_set(NULL, CONFIG_LOG_DOMAIN_ID, log_source_id_get(STRINGIFY(LOG_MODULE_NAME)), level);
#endif
...
int32_t mdm_hl7812_send_certificate(uint8_t const *certificate)
{
   int                   ret;
   size_t                send_len;
   char                  buf[sizeof("AT+KCERTSTORE=0,####")];
   struct hl7812_socket *sock = NULL;

   LOG_DBG("mdm_hl7812_send_certificate");

   /* Check parameters */
   if(certificate == NULL)
   {
      ret = -EINVAL;
      LOG_ERR("Err certificate");
      goto fail;
   }
   send_len = strlen(certificate);

   /* get socket */
   sock     = socket_from_id(MDM_HTTP_SOCKET_ID);
   if(sock == NULL)
   {
      sock = socket_http_create();
   }
   if(sock == NULL)
   {
      ret = -EINVAL;
   ...
   /* Send command KCERTSTORE */
   snprintk(buf, sizeof(buf), "AT+KCERTSTORE=0,%d", send_len);
   ret = send_at_cmd(sock, buf, K_NO_WAIT, 0, false);
   if(ret)
   {
      LOG_ERR("Err sending AT+KCERTSTORE (%d)", ret);
      goto fail;
   }

   /* wait for CONNECT or error */
   LOG_DBG("Wait for CONNECT");
   k_sem_reset(&sock->sock_send_sem);
   ...
   if(ret != 0)
   {
      LOG_ERR("Err sock waiting for CONNECT (%d)", ret);
      goto fail;
   }

   /* Send certificate */
   ret = mdm_receiver_send(&ictx.mdm_ctx, certificate, send_len);
   if(ret)
   {
      LOG_ERR("Err sending certificate (%d)", ret);
      goto fail;
   }
   LOG_DBG("Sent %zu bytes", send_len);
...
fail:
   LOG_ERR("Err mdm_hl7812_send_certificate (%d)", ret);
   return ret;
}
...
int32_t mdm_hl7812_send_sslcrypto(void)
{
   ...
   LOG_DBG("mdm_hl7812_send_crypto");
   ...
   /* Send command KSSLCRYPTO */
   ret         = send_at_cmd(sock, "AT+KSSLCRYPTO=0,8,3,25392,12,4,1", K_NO_WAIT, 0, false);
   if(ret)
   {
      LOG_ERR("Err sending AT+KSSLCRYPTO (%d)", ret);
      goto fail;
   }
...
fail:
   LOG_ERR("Err mdm_hl7812_send_sslcrypto (%d)", ret);
   return ret;
}
...
int32_t mdm_hl7812_set_apn(uint8_t const *apn)
{
   ...
   size_t                apn_len;
   struct hl7812_socket *sock                                                                 = NULL;
   char                  buf1[sizeof("AT+KCNXCFG=1,\"GPRS\",\"\"") + MDM_HL7812_APN_MAX_SIZE] = {0};
   char                  buf2[sizeof("AT+CGDCONT=1,\"IP\",\"\"") + MDM_HL7812_APN_MAX_SIZE]   = {0};

   LOG_DBG("mdm_hl7812_set_apn");
   ...
   /* Send command KCNXCFG */
   snprintk(buf1, sizeof("AT+KCNXCFG=1,\"GPRS\",\"\"") + strlen(apn), "AT+KCNXCFG=1,\"GPRS\",\"%s\"", apn);
   ret = send_at_cmd(sock, buf1, K_NO_WAIT, 0, false);
   if(ret)
   {
      LOG_ERR("Err sending AT+KCNXCFG (%d)", ret);
      goto fail;
   }
   ...
   if(ret)
   {
      LOG_ERR("Err sending AT+KCNXCFG: no OK (%d)", ret);
      goto fail;
   }

   /* Send command CGDCONT */
   snprintk(buf2, sizeof("AT+CGDCONT=1,\"IP\",\"\"") + strlen(apn), "AT+CGDCONT=1,\"IP\",\"%s\"", apn);
   ret = send_at_cmd(sock, buf2, K_NO_WAIT, 0, false);
   if(ret)
   {
      LOG_ERR("Err sending AT+CGDCONT (%d)", ret);
      goto fail;
   }
...
fail:
   LOG_ERR("Err mdm_hl7812_set_apn (%d)", ret);
   return ret;
}
...
int32_t mdm_hl7812_set_http_cfg(uint8_t const *url_domain)
{
   int                   ret;
   struct hl7812_socket *sock = NULL;
   char                  buf[sizeof("AT+KHTTPCFG=1,\"\",80,0,,,,,0") + MDM_HL7812_MAX_URL_DOMAIN_SIZE] = {0};

   LOG_DBG("mdm_hl7812_set_http_cfg");

   /* Check parameters */
   if(url_domain == NULL)
   {
      ret = -EINVAL;
      LOG_ERR("Err http cfg");
      goto fail;
   }
   ...
   /* Send command KHTTPCFG: Port 80, http */
   snprintk(buf,
            sizeof("AT+KHTTPCFG=1,\"\",80,0,,,,,0") + strlen(url_domain),
            "AT+KHTTPCFG=1,\"%s\",80,0,,,,,0",
            url_domain);
   ret = send_at_cmd(sock, buf, K_NO_WAIT, 0, false);
   if(ret)
   {
      LOG_ERR("Err sending AT+KHTTPCFG (%d)", ret);
      goto fail;
   }
   ...
   if(ret)
   {
      LOG_ERR("Err sending AT+KHTTPCFG: no OK (%d)", ret);
      goto fail;
   }
...
fail:
   LOG_ERR("Err mdm_hl7812_set_http_cfg (%d)", ret);
   return ret;
}
...
int32_t mdm_hl7812_send_http_header(uint8_t const *data)
{
   ...
   LOG_DBG("mdm_hl7812_send_http_header");

   /* Check parameters */
   if(data == NULL)
   {
      ret = -EINVAL;
      LOG_ERR("Err https header param");
      goto fail;
   }
   ...
   /* Send command KHTTPHEADER */
   ret         = send_at_cmd(sock, "AT+KHTTPHEADER=1", K_NO_WAIT, 0, false);
   if(ret)
   {
      LOG_ERR("Err sending AT+KHTTPHEADER (%d)", ret);
      goto fail;
   }
   ...
   /* Send header text */
   ret = mdm_receiver_send(&ictx.mdm_ctx, data, strlen(data));
   if(ret)
   {
      LOG_ERR("Err sending header text (%d)", ret);
      goto fail;
   }
   LOG_DBG("Sent header text %zu bytes", strlen(data));
...
fail:
   LOG_ERR("Err mdm_hl7812_send_http_header (%d)", ret);
   return ret;
}
...
int32_t mdm_hl7812_send_http_post(uint8_t const *url_path, uint8_t const *data)
{
   int                   ret;
   struct hl7812_socket *sock                                                               = NULL;
   char                  buf[sizeof("AT+KHTTPPOST=1,,\"\"") + MDM_HL7812_MAX_URL_PATH_SIZE] = {0};

   LOG_DBG("mdm_hl7812_send_http_post");

   /* Check parameters */
   if((url_path == NULL) || (data == NULL))
   {
      ret = -EINVAL;
      LOG_ERR("Err http post param");
      goto fail;
   }
   ...
   /* Send command AT+KHTTPPOST */
   snprintk(buf, sizeof(buf) + strlen(url_path), "AT+KHTTPPOST=1,,\"%s\"", url_path);
   ret = send_at_cmd(sock, buf, K_NO_WAIT, 0, false);
   if(ret)
   {
      LOG_ERR("Err sending AT+KHTTPPOST (%d)", ret);
      goto fail;
   }
   ...
   if(ret)
   {
      LOG_ERR("Err post data (%d)", ret);
      goto fail;
   }
   LOG_DBG("Sent %zu bytes", strlen(data));
...
fail:
   LOG_ERR("Err mdm_hl7812_send_http_post (%d)", ret);
   return ret;
}
...
int32_t mdm_hl7812_send_http_get(uint8_t const *url_path)
{
   int                   ret;
   struct hl7812_socket *sock                                                             = NULL;
   char                  buf[sizeof("AT+KHTTPGET=1,\"\",1") + MDM_HL7812_MAX_URL_PATH_SIZE] = {0};
   LOG_DBG("mdm_hl7812_send_http_get");
   ...
   {
      ret = -EINVAL;
      LOG_ERR("Err http get url_path");
      goto fail;
   }

   /* Send command AT+KHTTPGET */
   snprintk(buf, sizeof(buf) + strlen(url_path), "AT+KHTTPGET=1,\"%s\",1", url_path);
   ret = send_at_cmd(sock, buf, K_NO_WAIT, 0, false);
   if(ret)
   {
      LOG_ERR("Err sending AT+KHTTPGET (%d)", ret);
      goto fail;
   }
   ...
   if(ret)
   {
      LOG_ERR("Err sending AT+KHTTPGET: no CONNECT (%d)", ret);
      goto fail;
   }
   ...
   if(ret)
   {
      LOG_ERR("Err sending AT+KHTTPGET: no OK (%d)", ret);
      goto fail;
   }
...
fail:
   LOG_ERR("Err mdm_hl7812_send_http_get (%d)", ret);
   return ret;
}
...
int32_t mdm_hl7812_close_http(void)
{
   int                   ret;
   struct hl7812_socket *sock                            = NULL;
   char                  buf1[sizeof("AT+KHTTPCLOSE=1")] = {0};
   char                  buf2[sizeof("AT+KHTTPDEL=1")]   = {0};

   LOG_DBG("mdm_hl7812_close_http");
   ...
   /* Send command KHTTPCLOSE */
   snprintk(buf1, sizeof("AT+KHTTPCLOSE=1"), "AT+KHTTPCLOSE=1");
   ret = send_at_cmd(sock, buf1, K_NO_WAIT, 0, false);
   if(ret)
   {
      LOG_ERR("Err sending AT+KHTTPCLOSE (%d)", ret);
   }
   ...
   if(ret)
   {
      LOG_ERR("Err sending AT+KHTTPCLOSE: no OK (%d)", ret);
   }

   /* Send command KHTTPDEL */
   snprintk(buf2, sizeof("AT+KHTTPDEL=1"), "AT+KHTTPDEL=1");
   ret = send_at_cmd(sock, buf2, K_NO_WAIT, 0, false);
   if(ret)
   {
      LOG_ERR("Err sending AT+KHTTPDEL (%d)", ret);
   }
   ...
   if(ret)
   {
      LOG_ERR("Err sending AT+KHTTPDEL: no OK (%d)", ret);
   }
   sock->state = SOCK_IDLE;
...
fail:
   LOG_ERR("Err mdm_hl7812_close_http (%d)", ret);
   return ret;
}

/* Handler: GET HTTP */
static bool on_cmd_get_http(struct net_buf **buf, uint16_t len)
{
   char rcv_buf[CONFIG_MODEM_HL7812_RECV_BUF_SIZE] = {0};

   /* Process receiving data */
   if(len > 17)
   {
      net_buf_linearize(rcv_buf, CONFIG_MODEM_HL7812_RECV_BUF_SIZE, *buf, 0, len - 17);
   }
   else
   {
      net_buf_linearize(rcv_buf, CONFIG_MODEM_HL7812_RECV_BUF_SIZE, *buf, 0, len);
   }
   LOG_INF("Received message: %s", rcv_buf);

   return true;
}

static int send_data(struct hl7812_socket *sock, struct net_pkt *pkt)
{
   int             ret;
   struct net_buf *frag;
   ...
   {
      if(!net_addr_ntop(sock->family, &net_sin(&sock->dst)->sin_addr, dst_addr, sizeof(dst_addr)))
      {
 ...
 */
static int pkt_setup_ip_data(struct net_pkt *pkt, struct hl7812_socket *sock)
{
   int      hdr_len  = 0;
...
static uint32_t wait_for_modem_data(struct net_buf **buf, uint32_t current_len, uint32_t expected_len)
{
   ...
   while((current_len < expected_len) && (waitForDataTries < MDM_WAIT_FOR_DATA_RETRIES))
   {
      ...
      current_len += hl7812_read_rx(buf);
      waitForDataTries++;
   }
...
static uint32_t
        wait_for_modem_data_and_newline(struct net_buf **buf, uint32_t current_len, uint32_t expected_len)
{
...
static bool on_cmd_atcmdinfo_manufacturer(struct net_buf **buf, uint16_t len)
{
   ...
   out_len = net_buf_linearize(ictx.mdm_manufacturer, sizeof(ictx.mdm_manufacturer) - 1, *buf, 0, len);
...
static bool on_cmd_atcmdinfo_revision(struct net_buf **buf, uint16_t len)
{
   ...
   /* make sure revision data is received
    *  waiting for something like: AHL7812.1.2.3.1.20171211\r\n
    */
   wait_for_modem_data_and_newline(buf, net_buf_frags_len(*buf), MDM_HL7812_REVISION_MAX_SIZE);
   ...
   else if(len > MDM_HL7812_REVISION_MAX_STRLEN)
   {
      LOG_WRN("revision too long (len:%d)", len);
      len = MDM_HL7812_REVISION_MAX_STRLEN;
   }
...
static bool on_cmd_atcmdinfo_imei(struct net_buf **buf, uint16_t len)
{
    ...
    *  waiting for: ###############\r\n
    */
   wait_for_modem_data_and_newline(buf, net_buf_frags_len(*buf), MDM_HL7812_IMEI_SIZE);
   ...
   if(len < MDM_HL7812_IMEI_STRLEN)
   {
      LOG_WRN("IMEI too short (len:%d)", len);
   }
   else if(len > MDM_HL7812_IMEI_STRLEN)
   {
      LOG_WRN("IMEI too long (len:%d)", len);
      len = MDM_HL7812_IMEI_STRLEN;
   }
...
static bool on_cmd_atcmdinfo_iccid(struct net_buf **buf, uint16_t len)
{
    ...
    *  waiting for: <ICCID>\r\n
    */
   wait_for_modem_data_and_newline(buf, net_buf_frags_len(*buf), MDM_HL7812_ICCID_SIZE);
   ...
   if(len > MDM_HL7812_ICCID_STRLEN)
   {
      LOG_WRN("ICCID too long (len:%d)", len);
      len = MDM_HL7812_ICCID_STRLEN;
   }

   out_len                 = net_buf_linearize(ictx.mdm_iccid, MDM_HL7812_ICCID_STRLEN, *buf, 0, len);
...
static bool on_cmd_atcmdinfo_imsi(struct net_buf **buf, uint16_t len)
{
    ...
    *  waiting for: <IMSI>\r\n
    */
   wait_for_modem_data_and_newline(buf, net_buf_frags_len(*buf), MDM_HL7812_IMSI_MIN_STR_SIZE);
   ...
   if(len > MDM_HL7812_IMSI_MAX_STRLEN)
   {
      LOG_WRN("IMSI too long (len:%d)", len);
      len = MDM_HL7812_IMSI_MAX_STRLEN;
   }

   out_len                = net_buf_linearize(ictx.mdm_imsi, MDM_HL7812_IMSI_MAX_STR_SIZE, *buf, 0, len);
...
static void dns_work_cb(struct k_work *work)
{
   ...
   static char const *const    dns_servers_str[] = {ictx.dns_v4_string,
   #ifdef CONFIG_NET_IPV6
                                                 ictx.dns_v6_string,
   #endif
                                                 NULL};
   ...
   if(ictx.iface && net_if_is_up(ictx.iface))
   {
      ...
      ret    = dns_resolve_reconfigure(dnsCtx, (char const **)dns_servers_str, NULL);
      if(ret < 0)
      {
...
{
   return ictx.network_state;
}

void mdm_hl7812_get_ip_addr_v4(struct in_addr *ip_addr)
{
   /* store the new IP addr */
   net_ipaddr_copy(ip_addr, &ictx.ipv4Addr);

   return;
}

/* Convert HL7812 IPv6 address string in format
 * a01.a02.a03.a04.a05.a06.a07.a08.a09.a10.a11.a12.a13.a14.a15.a16 to
 * an IPv6 address.
 */
static int hl7812_net_addr6_pton(char const *src, struct in6_addr *dst)
{
   int      num_sections = 8;
   int      i, len;
...
static bool on_cmd_atcmdinfo_ipaddr(struct net_buf **buf, uint16_t len)
{
   ...
   bool            is_ipv4;
   int             addr_len;
   char            temp_addr_str[HL7812_IPV6_ADDR_LEN];
   k_timeout_t     delay;
   ...
   {
      ret = hl7812_net_addr6_pton(temp_addr_str, &new_ipv6_addr);
   }
   if(ret < 0)
   {
   ...
   {
      /* store HL7812 formatted IPv6 DNS string temporarily */
      strncpy(ictx.dns_v6_string, addr_start, addr_len);

      ret = hl7812_net_addr6_pton(ictx.dns_v6_string, &ictx.dns_v6);
      net_addr_ntop(AF_INET6, &ictx.dns_v6, ictx.dns_v6_string, sizeof(ictx.dns_v6_string));
   ...
   if(ictx.iface)
   {
      ...
      else
      {
         ...
         if(!net_if_ipv6_addr_add(ictx.iface, &new_ipv6_addr, NET_ADDR_AUTOCONF, 0))
         {
      ...
      k_work_reschedule_for_queue(&hl7812_workq, &ictx.dns_work, delay);
   }
   else
   {
      LOG_ERR("iface NULL");
...
static bool on_cmd_atcmdinfo_serial_number(struct net_buf **buf, uint16_t len)
{
   ...
   if(sn_len < MDM_HL7812_SERIAL_NUMBER_STRLEN)
   {
      LOG_WRN("sn too short (len:%d)", sn_len);
   }
   else if(sn_len > MDM_HL7812_SERIAL_NUMBER_STRLEN)
   {
      LOG_WRN("sn too long (len:%d)", sn_len);
      sn_len = MDM_HL7812_SERIAL_NUMBER_STRLEN;
   }
...
static bool on_cmd_radio_tech_status(struct net_buf **buf, uint16_t len)
{
   ...
   LOG_INF("+KSRAT: %d (0:CAT-M1, 1:NB-IoT and 2:GSM)", ictx.mdm_rat);
   event_handler(HL7812_EVENT_RAT, &ictx.mdm_rat);
...
static bool on_cmd_radio_band_configuration(struct net_buf **buf, uint16_t len)
{
   ...
   memcpy(ictx.mdm_bands_string, &value[MDM_TOP_BAND_START_POSITION], MDM_HL7812_LTE_BAND_STRLEN);
...
static bool on_cmd_radio_active_bands(struct net_buf **buf, uint16_t len)
{
   ...
   memcpy(ictx.mdm_active_bands_string, &value[MDM_TOP_BAND_START_POSITION], MDM_HL7812_LTE_BAND_STRLEN);
   event_handler(HL7812_EVENT_ACTIVE_BANDS, ictx.mdm_active_bands_string);
...
static char *get_startup_state_string(enum mdm_hl7812_startup_state state)
{
   /* clang-format off */
	switch (state) {
		PREFIXED_SWITCH_CASE_RETURN_STRING(HL7812_STARTUP_STATE, READY);
		PREFIXED_SWITCH_CASE_RETURN_STRING(HL7812_STARTUP_STATE, WAITING_FOR_ACCESS_CODE);
		PREFIXED_SWITCH_CASE_RETURN_STRING(HL7812_STARTUP_STATE, SIM_NOT_PRESENT);
		PREFIXED_SWITCH_CASE_RETURN_STRING(HL7812_STARTUP_STATE, SIMLOCK);
		PREFIXED_SWITCH_CASE_RETURN_STRING(HL7812_STARTUP_STATE, UNRECOVERABLE_ERROR);
		PREFIXED_SWITCH_CASE_RETURN_STRING(HL7812_STARTUP_STATE, UNKNOWN);
		PREFIXED_SWITCH_CASE_RETURN_STRING(HL7812_STARTUP_STATE, INACTIVE_SIM);
	default:
		return "UNKNOWN";
	}
   /* clang-format on */
...
static void generate_startup_state_event(void)
{
   struct mdm_hl7812_compound_event event;
   ...
   event_handler(HL7812_EVENT_STARTUP_STATE_CHANGE, &event);
}
...
#if CONFIG_MODEM_HL7812_LOW_POWER_MODE
   switch(level)
   {
      case HL7812_SLEEP_AWAKE:
      case HL7812_SLEEP_HIBERNATE:
      case HL7812_SLEEP_LITE_HIBERNATE:
      case HL7812_SLEEP_SLEEP:
...
static void initialize_sleep_level(void)
{
   if(ictx.desired_sleep_level == HL7812_SLEEP_UNINITIALIZED)
   {
      if(IS_ENABLED(CONFIG_MODEM_HL7812_SLEEP_LEVEL_HIBERNATE))
      {
         ictx.desired_sleep_level = HL7812_SLEEP_HIBERNATE;
      }
      else if(IS_ENABLED(CONFIG_MODEM_HL7812_SLEEP_LEVEL_LITE_HIBERNATE))
      {
         ictx.desired_sleep_level = HL7812_SLEEP_LITE_HIBERNATE;
      }
      else if(IS_ENABLED(CONFIG_MODEM_HL7812_SLEEP_LEVEL_SLEEP))
      {
         ictx.desired_sleep_level = HL7812_SLEEP_SLEEP;
      }
      else
      {
         ictx.desired_sleep_level = HL7812_SLEEP_AWAKE;
      }
   }
}
...
static int set_sleep_level(void)
{
   ...
   static char const SLEEP_CMD_FMT[] = "AT+KSLEEP=%d,%d,%d";
   int               delay           = CONFIG_MODEM_HL7812_SLEEP_DELAY_AFTER_REBOOT;
   int               ret             = 0;

   /* AT+KSLEEP= <management>[,<level>[,<delay to sleep after reboot>]]
    * management 1 means the HL7812 decides when it enters sleep mode
    */
...
#endif /* CONFIG_MODEM_HL7812_LOW_POWER_MODE */

static char *get_sleep_state_string(enum mdm_hl7812_sleep state)
{
   /* clang-format off */
	switch (state) {
		PREFIXED_SWITCH_CASE_RETURN_STRING(HL7812_SLEEP, UNINITIALIZED);
		PREFIXED_SWITCH_CASE_RETURN_STRING(HL7812_SLEEP, HIBERNATE);
		PREFIXED_SWITCH_CASE_RETURN_STRING(HL7812_SLEEP, LITE_HIBERNATE);
		PREFIXED_SWITCH_CASE_RETURN_STRING(HL7812_SLEEP, SLEEP);
		PREFIXED_SWITCH_CASE_RETURN_STRING(HL7812_SLEEP, AWAKE);
	default:
		return "UNKNOWN";
	}
   /* clang-format on */
...
static void set_sleep_state(enum mdm_hl7812_sleep state)
{
   ...
   if(ictx.sleep_state != HL7812_SLEEP_AWAKE)
   {
...
static void generate_sleep_state_event(void)
{
   ...
   event_handler(HL7812_EVENT_SLEEP_STATE_CHANGE, &event);
}
...
static char *get_fota_state_string(enum mdm_hl7812_fota_state state)
{
   /* clang-format off */
	switch (state) {
		PREFIXED_SWITCH_CASE_RETURN_STRING(HL7812_FOTA, IDLE);
		PREFIXED_SWITCH_CASE_RETURN_STRING(HL7812_FOTA, START);
		PREFIXED_SWITCH_CASE_RETURN_STRING(HL7812_FOTA, WIP);
		PREFIXED_SWITCH_CASE_RETURN_STRING(HL7812_FOTA, PAD);
		PREFIXED_SWITCH_CASE_RETURN_STRING(HL7812_FOTA, SEND_EOT);
		PREFIXED_SWITCH_CASE_RETURN_STRING(HL7812_FOTA, FILE_ERROR);
		PREFIXED_SWITCH_CASE_RETURN_STRING(HL7812_FOTA, INSTALL);
		PREFIXED_SWITCH_CASE_RETURN_STRING(HL7812_FOTA, REBOOT_AND_RECONFIGURE);
		PREFIXED_SWITCH_CASE_RETURN_STRING(HL7812_FOTA, COMPLETE);
	default:
		return "UNKNOWN";
	}
   /* clang-format on */
...
static void set_fota_state(enum mdm_hl7812_fota_state state)
{
   LOG_INF("FOTA state: %s->%s", get_fota_state_string(ictx.fw_update_state), get_fota_state_string(state));
...
static void generate_fota_state_event(void)
{
   ...
   event_handler(HL7812_EVENT_FOTA_STATE, &event);
}
...
static void generate_fota_count_event(void)
{
   ...
   event_handler(HL7812_EVENT_FOTA_COUNT, &count);
}
#endif
...
static bool on_cmd_startup_report(struct net_buf **buf, uint16_t len)
{
   ...
   {
      set_startup_state(HL7812_STARTUP_STATE_UNKNOWN);
   }
   ...
   if(ictx.fw_updated)
   {
      ...
      set_fota_state(HL7812_FOTA_REBOOT_AND_RECONFIGURE);
...
static bool profile_handler(struct net_buf **buf, uint16_t len, bool active_profile)
{
   ...
   if(line_length)
   {
      ...
      output_length = net_buf_linearize(line, SIZE_WITHOUT_NUL(line), *buf, 0, line_length);
...
static bool on_cmd_atcmdinfo_pdp_authentication_cfg(struct net_buf **buf, uint16_t len)
{
   struct net_buf *frag = NULL;
   uint16_t        line_length;
   char            line[MDM_HL7812_APN_CMD_MAX_SIZE];
   size_t          output_length;
   size_t          i;
   char           *p;

   wait_for_modem_data_and_newline(buf, net_buf_frags_len(*buf), MDM_HL7812_APN_CMD_MAX_SIZE);
   ...
   if(line_length)
   {
      ...
      if(output_length > 0)
      {
         ...
         if(p != NULL)
         {
            p += 1;
            i = 0;
            while((p != NULL) && (*p != '"') && (i < MDM_HL7812_APN_USERNAME_MAX_STRLEN))
            {
         ...
         if(p != NULL)
         {
            p += 1;
            i = 0;
            while((p != NULL) && (*p != '"') && (i < MDM_HL7812_APN_PASSWORD_MAX_STRLEN))
            {
   ...
   if(line_length)
   {
      ...
      if(output_length > 0)
      {
         ...
         if(*p == '"')
         {
            p++;
            i = 0;
            while((p != NULL) && (*p != '"') && (i < MDM_HL7812_APN_MAX_STRLEN))
            {
...
static int hl7812_query_rssi(void)
{
   int ret;
...
static void hl7812_start_rssi_work(void)
{
   /* Rate is not checked here to allow one reading
    * when going from network down->up
    */
   k_work_reschedule_for_queue(&hl7812_workq, &ictx.rssi_query_work, K_NO_WAIT);
}

static void hl7812_stop_rssi_work(void)
{
   int rc;
...
static void hl7812_rssi_query_work(struct k_work *work)
{
   rssi_query();
   ...
   if(CONFIG_MODEM_HL7812_RSSI_RATE_SECONDS > 0)
   {
      k_work_reschedule_for_queue(&hl7812_workq,
                                  ...
                                  K_SECONDS(CONFIG_MODEM_HL7812_RSSI_RATE_SECONDS));
   }
}
...
static bool on_cmd_gps_event(struct net_buf **buf, uint16_t len)
{
   ...
   if(event == HL7812_GNSS_EVENT_POSITION)
   {
      event_handler(HL7812_EVENT_GPS_POSITION_STATUS, &status);
   }
 ...
 */
static bool gps_handler(struct net_buf **buf, uint16_t len, enum mdm_hl7812_gps_string_types str_type)
{
   struct mdm_hl7812_compound_event event;
   char                             gps_str[MDM_HL7812_MAX_GPS_STR_SIZE];
   ...
   event_handler(HL7812_EVENT_GPS, &event);
done:
   return true;
}
...
static bool on_cmd_polte_registration(struct net_buf **buf, uint16_t len)
{
   ...
   struct mdm_hl7812_polte_registration_event_data data;
   struct net_buf                                 *frag = NULL;
   size_t                                          out_len;
   char                                           *location;
   bool                                            parsed;
   ...
   event_handler(HL7812_EVENT_POLTE_REGISTRATION, &data);
...
static bool on_cmd_polte_locate_cmd_rsp(struct net_buf **buf, uint16_t len)
{
   ...
   size_t                                out_len;
   struct net_buf                       *frag = NULL;
   struct mdm_hl7812_polte_location_data data;
   ...
   event_handler(HL7812_EVENT_POLTE_LOCATE_STATUS, &data);
...
static bool on_cmd_polte_location(struct net_buf **buf, uint16_t len)
{
   ...
   bool                                  parsed;
   struct mdm_hl7812_polte_location_data data;
   static char const                     POLTE_LOC_DELIMITER[] = "\",\"";
   ...
   do
   {
      ...
      if(data.status != 0)
      {
         ...
         data.status = MDM_HL7812_POLTE_SERVER_ERROR;
         parsed      = true;
         break;
      }
      ...
      if(start > rsp && start < rsp_end && end < rsp_end && end > start)
      {
         memcpy(data.confidence_in_meters, start, MIN(end - start, sizeof(data.confidence_in_meters) - 1));
      }
      else
      {
         break;
   ...
   event_handler(HL7812_EVENT_POLTE, &data);
...
static void notify_all_tcp_sockets_closed(void)
{
   ...
   for(i = 0; i < MDM_MAX_SOCKETS; i++)
   {
      ...
      if((sock->context != NULL) && (sock->type == SOCK_STREAM))
      {
         ...
         if(sock->recv_cb)
         {
            sock->recv_cb(sock->context, sock->recv_pkt, NULL, NULL, 0, sock->recv_user_data);
         }
      }
   }
}
...
static void iface_status_work_cb(struct k_work *work)
{
   ...
   if(!ictx.initialized && ictx.restarting)
   {
      ...
      k_work_reschedule_for_queue(&hl7812_workq, &ictx.iface_status_work, IFACE_WORK_DELAY);
      goto done;
   }
   ...
   {
      case HL7812_HOME_NETWORK:
      case HL7812_ROAMING:
         if(ictx.iface && !net_if_is_up(ictx.iface))
         {
            LOG_DBG("HL7812 iface UP");
            net_if_up(ictx.iface);
         }
         break;
      case HL7812_OUT_OF_COVERAGE:
      default:
         if(ictx.iface && net_if_is_up(ictx.iface) && (ictx.low_power_mode != HL7812_LPM_PSM))
         {
            LOG_DBG("HL7812 iface DOWN");
   ...
   if((ictx.iface && !net_if_is_up(ictx.iface)) ||
      (ictx.low_power_mode == HL7812_LPM_PSM && ictx.network_state == HL7812_OUT_OF_COVERAGE))
   {
      hl7812_stop_rssi_work();
      notify_all_tcp_sockets_closed();
   }
   else if(ictx.iface && net_if_is_up(ictx.iface))
   {
      hl7812_start_rssi_work();
      /* get IP address info */
      (void)send_at_cmd(NULL,
                        "AT+CGCONTRDP=1",
                        MDM_CMD_SEND_TIMEOUT,
                        CONFIG_MODEM_HL7812_GET_IP_ADDR_INFO_ATTEMPTS,
                        false);
      /* get active bands */
...
static char *get_network_state_string(enum mdm_hl7812_network_state state)
{
   switch(state)
   {
      PREFIXED_SWITCH_CASE_RETURN_STRING(HL7812, NOT_REGISTERED);
      PREFIXED_SWITCH_CASE_RETURN_STRING(HL7812, HOME_NETWORK);
      PREFIXED_SWITCH_CASE_RETURN_STRING(HL7812, SEARCHING);
      PREFIXED_SWITCH_CASE_RETURN_STRING(HL7812, REGISTRATION_DENIED);
      PREFIXED_SWITCH_CASE_RETURN_STRING(HL7812, OUT_OF_COVERAGE);
      PREFIXED_SWITCH_CASE_RETURN_STRING(HL7812, ROAMING);
      PREFIXED_SWITCH_CASE_RETURN_STRING(HL7812, EMERGENCY);
      PREFIXED_SWITCH_CASE_RETURN_STRING(HL7812, UNABLE_TO_CONFIGURE);
      default:
         return "UNKNOWN";
   }
}
...
static void generate_network_state_event(void)
{
   ...
   event_handler(HL7812_EVENT_NETWORK_STATE_CHANGE, &event);
}
...
{
   struct net_buf *frag = NULL;
   char            carrier[MDM_HL7812_OPERATOR_INDEX_SIZE];
   size_t          out_len;

   wait_for_modem_data_and_newline(buf, net_buf_frags_len(*buf), MDM_HL7812_OPERATOR_INDEX_SIZE);
   ...
   out_len             = net_buf_linearize(carrier, MDM_HL7812_OPERATOR_INDEX_STRLEN, *buf, 0, len);
   carrier[out_len]    = 0;
...
static bool on_cmd_modem_functionality(struct net_buf **buf, uint16_t len)
{
   struct net_buf *frag;
   size_t          out_len;
   char            rsp[MDM_HL7812_MODEM_FUNCTIONALITY_SIZE];

   wait_for_modem_data_and_newline(buf, net_buf_frags_len(*buf), MDM_HL7812_MODEM_FUNCTIONALITY_SIZE);
   ...
   out_len            = net_buf_linearize(rsp, MDM_HL7812_MODEM_FUNCTIONALITY_STRLEN, *buf, 0, len);
   rsp[out_len]       = 0;
...
static bool on_cmd_survey_status(struct net_buf **buf, uint16_t len)
{
   ...
   size_t          out_len;
   char           *value;
   struct mdm_hl7812_site_survey site_survey;
   ...
   event_handler(HL7812_EVENT_SITE_SURVEY, &site_survey);
...
static bool on_cmd_rtc_query(struct net_buf **buf, uint16_t len)
{
   ...
   else
   {
      ...
      ictx.local_time_valid =
              convert_time_string_to_struct(&ictx.local_time, &ictx.local_time_offset, rtc_string);
   }
done:
   return true;
}

static bool valid_time_string(char const *time_string)
{
   size_t offset, i;
...
static bool convert_time_string_to_struct(struct tm *tm, int32_t *offset, char *time_string)
{
   ...
   tm->tm_year  = TIME_STRING_TO_TM_STRUCT_YEAR_OFFSET + get_next_time_string_digit(&fc, &ptr, TM_YEAR_RANGE);
   ...
   tm->tm_isdst = 0;
   *offset = (int32_t)get_next_time_string_digit(&fc, &ptr, QUARTER_HOUR_RANGE) * SECONDS_PER_QUARTER_HOUR;
...
static bool on_cmd_network_report(struct net_buf **buf, uint16_t len)
{
   ...
   out_len = net_buf_linearize(ictx.mdm_network_status, sizeof(ictx.mdm_network_status) - 1, *buf, 0, len);
   ...
   /* keep HL7812 awake because we want to process the network state soon */
   allow_sleep(false);
   /* start work to adjust iface */
   k_work_reschedule_for_queue(&hl7812_workq, &ictx.iface_status_work, IFACE_WORK_DELAY);
...
static bool on_cmd_atcmdinfo_rssi(struct net_buf **buf, uint16_t len)
{
   ...
   {
      LOG_INF("RSSI (RSRP): %d SINR: %d", ictx.mdm_rssi, ictx.mdm_sinr);
      event_handler(HL7812_EVENT_RSSI, &ictx.mdm_rssi);
      event_handler(HL7812_EVENT_SINR, &ictx.mdm_sinr);
   }
done:
...
static bool on_cmd_sock_ind(struct net_buf **buf, uint16_t len, char const *const type)
{
   struct hl7812_socket *sock = NULL;
   char                 *delim;
   char                  value[MDM_MAX_RESP_SIZE];
...
static void sock_notif_cb_work(struct k_work *work)
{
   ...
   sock  = CONTAINER_OF(dwork, struct hl7812_socket, notif_work);
   ...
   if(sock->recv_pkt != NULL)
   {
       ...
       */
      k_work_reschedule_for_queue(&hl7812_workq, &sock->notif_work, MDM_SOCK_NOTIF_DELAY);
   }
   else
   {
      if(sock->type == SOCK_STREAM)
      {
         ...
         k_work_submit_to_queue(&hl7812_workq, &sock->recv_cb_work);
      }
   }
   hl7812_unlock();
}
...
static bool on_cmd_sock_notif(struct net_buf **buf, uint16_t len)
{
   ...
   switch(notif_val)
   {
      case HL7812_TCP_DATA_SND:
         err         = false;
         sock->error = 0;
         break;
      case HL7812_TCP_DISCON:
         trigger_sem = false;
         err         = true;
         sock->error = -ENOTCONN;
         break;
...
static int delete_socket(struct hl7812_socket *sock, enum net_sock_type type, uint8_t id)
{
...
static void sockreadrecv_cb_work(struct k_work *work)
{
   ...
   sock = CONTAINER_OF(work, struct hl7812_socket, recv_cb_work);

   LOG_DBG("Sock %d RX CB (size: %zd)",
           sock->socket_id,
...
static void sock_read(struct net_buf **buf, uint16_t len)
{
   ...
   /* allocate an RX pkt */
   sock->recv_pkt = net_pkt_rx_alloc_with_buffer(net_context_get_iface(sock->context),
                                                 sock->rx_size,
                                                 sock->family,
                                                 sock->ip_proto,
                                                 BUF_ALLOC_TIMEOUT);
...
static int start_socket_rx(struct hl7812_socket *sock, uint16_t rx_size)
{
   char sendbuf[sizeof("AT+KTCPRCV=+#########,#####")];

   if((sock->socket_id <= 0) || (sock->rx_size <= 0))
   {
      LOG_WRN("Cannot start socket RX, ID: %d rx size: %d", sock->socket_id, sock->rx_size);
      return -1;
   }
...
#endif
      snprintk(sendbuf, sizeof(sendbuf), "AT+KTCPRCV=%d,%u", sock->socket_id, sock->rx_size);
   }
...
static void sock_rx_data_cb_work(struct k_work *work)
{
   ...
   sock = CONTAINER_OF(work, struct hl7812_socket, rx_data_work);
...
static bool on_cmd_sockdataind(struct net_buf **buf, uint16_t len)
{
   ...
   *delim++   = '\0';
   ...
   if(defer_rx)
   {
      ...
      k_work_submit_to_queue(&hl7812_workq, &sock->rx_data_work);
   }
   else
   {
      if(left_bytes > 0)
...
static bool on_cmd_device_service_ind(struct net_buf **buf, uint16_t len)
{
   ...
   {
      k_work_submit_to_queue(&hl7812_workq, &ictx.finish_fw_update_work);
   }
#endif
...
inline static struct net_buf *read_rx_allocator(k_timeout_t timeout, void *user_data)
{
...
static size_t hl7812_read_rx(struct net_buf **buf)
{
   uint8_t  uart_buffer[CONFIG_MODEM_HL7812_RECV_BUF_SIZE];
   ...
   while(true)
   {
      ret = mdm_receiver_recv(&ictx.mdm_ctx, uart_buffer, sizeof(uart_buffer), &bytes_read);
      ...
      if(IS_ENABLED(HL7812_ENABLE_VERBOSE_MODEM_RECV_HEXDUMP))
      {
         LOG_HEXDUMP_DBG((uint8_t const *)&uart_buffer, bytes_read, "HL7812 RX");
      }
      ...
      rx_len = net_buf_append_bytes(*buf,
                                    bytes_read,
                                    uart_buffer,
                                    BUF_ALLOC_TIMEOUT,
                                    read_rx_allocator,
...
static void finish_fw_update_work_callback(struct k_work *item)
{
   ...
   set_fota_state(HL7812_FOTA_INSTALL);
   hl7812_unlock();
}
...
static int send_fw_update_packet(struct xmodem_packet *pkt)
{
   ...
   return mdm_receiver_send(&ictx.mdm_ctx, (uint8_t const *)pkt, XMODEM_PACKET_SIZE);
}
...
static int prepare_and_send_fw_packet(void)
{
   ...
   if(ret < 0)
   {
      set_fota_state(HL7812_FOTA_FILE_ERROR);
   ...
   {
      set_fota_state(HL7812_FOTA_PAD);
...
static void process_fw_update_rx(struct net_buf **rx_buf)
{
   ...
   if(xm_msg == XM_NACK)
   {
      if(ictx.fw_update_state == HL7812_FOTA_START)
      {
         /* send first FW update packet */
         set_fota_state(HL7812_FOTA_WIP);
         ictx.file_pos           = 0;
      ...
      else if(ictx.fw_update_state == HL7812_FOTA_WIP)
      {
   ...
   else if(xm_msg == XM_ACK)
   {
      ...
      if(ictx.fw_update_state == HL7812_FOTA_WIP)
      {
         /* send next FW update packet */
         prepare_and_send_fw_packet();
      }
      else if(ictx.fw_update_state == HL7812_FOTA_PAD)
      {
         set_fota_state(HL7812_FOTA_SEND_EOT);
...
#endif /* CONFIG_MODEM_HL7812_FW_UPDATE */

/* RX thread */
static void hl7812_rx(void)
{
...
#ifdef HL7812_LOG_UNHANDLED_RX_MSGS
   ...
   while(true)
   {
      ...
      while(rx_buf)
      {
...
#ifdef CONFIG_MODEM_HL7812_FW_UPDATE
         if((ictx.fw_update_state == HL7812_FOTA_START) || (ictx.fw_update_state == HL7812_FOTA_WIP) ||
            (ictx.fw_update_state == HL7812_FOTA_PAD))
         {
         ...
         for(i = 0; i < ARRAY_SIZE(handlers); i++)
         {
            ...
            {
               cmp_res = strncmp(ictx.no_id_resp_cmd, handlers[i].cmd, handlers[i].cmd_len);
            }
            else
            {
               cmp_res = strncmp(rx_msg, handlers[i].cmd, handlers[i].cmd_len);
            }
            ...
            if(cmp_res == 0)
            {
               ...
               if(handlers[i].func)
               {
                  remove_line_from_buf = handlers[i].func(&rx_buf, len);
               }
               cmd_handled            = true;
               ictx.search_no_id_resp = false;
               frag                   = NULL;
         ...
         if(IS_ENABLED(HL7812_LOG_UNHANDLED_RX_MSGS) && !cmd_handled && frag && len > 1)
         {
            out_len      = net_buf_linearize(msg, sizeof(msg), rx_buf, 0, len);
            msg[out_len] = 0;
            LOG_HEXDUMP_DBG((uint8_t const *)&msg, len, "UNHANDLED RX");
         }
...
static void shutdown_uart(void)
{
   ...
   if(ictx.uart_on)
   {
      HL7812_IO_DBG_LOG("Power OFF the UART");
...
static void power_on_uart(void)
{
   ...
   if(!ictx.uart_on)
   {
      HL7812_IO_DBG_LOG("Power ON the UART");
...
{
   HL7812_IO_DBG_LOG("Preparing IO for reset/sleep");
   shutdown_uart();
   modem_assert_wake(false);
...
static void mdm_vgpio_work_cb(struct k_work *item)
{
   ...
   if(!ictx.vgpio_state)
   {
      ...
      if(ictx.iface && ictx.initialized && net_if_is_up(ictx.iface) && ictx.low_power_mode != HL7812_LPM_PSM)
      {
         net_if_down(ictx.iface);
      }
   }
...
{
   ictx.vgpio_state = read_pin(1, &hl7812_cfg.gpio[MDM_VGPIO]);
   HL7812_IO_DBG_LOG("VGPIO:%d", ictx.vgpio_state);
   if(!ictx.vgpio_state)
   {
    ...
    */
   k_work_submit_to_queue(&hl7812_workq, &ictx.mdm_vgpio_work);
}

void mdm_uart_dsr_callback_isr(const struct device *port, struct gpio_callback *cb, uint32_t pins)
{
   ictx.dsr_state = read_pin(1, &hl7812_cfg.gpio[MDM_UART_DSR]);
   HL7812_IO_DBG_LOG("MDM_UART_DSR:%d", ictx.dsr_state);
}
...
{
#ifdef CONFIG_MODEM_HL7812_LOW_POWER_MODE
   ictx.gpio6_state = read_pin(1, &hl7812_cfg.gpio[MDM_GPIO6]);
   HL7812_IO_DBG_LOG("MDM_GPIO6:%d", ictx.gpio6_state);
   if(!ictx.gpio6_state)
   {
      /* HL7812 is not awake, shut down UART to save power */
      shutdown_uart();
      ictx.wait_for_KSUP          = true;
      ictx.wait_for_KSUP_tries    = 0;
      ictx.reconfig_IP_connection = true;
   ...
   if((ictx.gpio6_callback != NULL) && ((ictx.desired_sleep_level == HL7812_SLEEP_HIBERNATE) ||
                                        (ictx.desired_sleep_level == HL7812_SLEEP_LITE_HIBERNATE)))
   {
   ...
   check_hl7812_awake();
#else
   HL7812_IO_DBG_LOG("Spurious gpio6 interrupt from the modem");
#endif
}
 ...
 */
static int glitch_filter(int                        default_state,
...
void mdm_uart_cts_callback(const struct device *port, struct gpio_callback *cb, uint32_t pins)
{
   ...
   ictx.cts_state = glitch_filter(0,
                                  &hl7812_cfg.gpio[MDM_UART_CTS],
                                  CONFIG_MODEM_HL7812_CTS_FILTER_US,
                                  CONFIG_MODEM_HL7812_CTS_FILTER_MAX_ITERATIONS);

   /* CTS toggles A LOT,
    * comment out the debug print unless we really need it.
    */
   /* HL7812_IO_DBG_LOG("MDM_UART_CTS:%d", ictx.cts_state); */

   if((ictx.cts_callback != NULL) && (ictx.desired_sleep_level == HL7812_SLEEP_SLEEP))
   {
   ...
   {
      power_on_uart();
      if(ictx.sleep_state == HL7812_SLEEP_SLEEP)
      {
         allow_sleep(false);
      }
   }
...
static void modem_reset(void)
{
   ...
   gpio_pin_set_dt(&hl7812_cfg.gpio[MDM_RESET], 1);
   /* >20 milliseconds required for reset low */
   k_sleep(MDM_RESET_LOW_TIME);

   ictx.mdm_startup_reporting_on = false;
   set_sleep_state(HL7812_SLEEP_UNINITIALIZED);
   check_hl7812_awake();
   set_network_state(HL7812_NOT_REGISTERED);
   set_startup_state(HL7812_STARTUP_STATE_UNKNOWN);
#ifdef CONFIG_MODEM_HL7812_FW_UPDATE
   if(ictx.fw_update_state == HL7812_FOTA_REBOOT_AND_RECONFIGURE)
   {
      set_fota_state(HL7812_FOTA_COMPLETE);
   }
   else
   {
      set_fota_state(HL7812_FOTA_IDLE);
   }
#endif
   k_sem_reset(&ictx.mdm_awake);
}
...
{
   LOG_INF("Modem Run");
   gpio_pin_set_dt(&hl7812_cfg.gpio[MDM_RESET], 0);
 ...
 */
static int compare_versions(char *v1, char const *v2)
{
   int           result = 0;
   char         *tail1;
   char         *tail2;
...
static int setup_gprs_connection(char *access_point_name)
{
   ...
   strncat(cmd_string, MODEM_HL7812_ADDRESS_FAMILY "\"", cmd_max_len);
   return send_at_cmd(NULL, cmd_string, MDM_CMD_SEND_TIMEOUT, 0, false);
}

static int set_bands(char const *bands, bool full_reboot)
{
   int  ret;
   ...
   if(!full_reboot)
   {
      ret = send_at_cmd(NULL, "AT+CFUN=1,1", MDM_CMD_SEND_TIMEOUT, MDM_DEFAULT_AT_CMD_RETRIES, false);
      if(ret < 0)
      {
         return ret;
      }
...
static int modem_reset_and_configure(void)
{
   int  ret   = 0;
   bool sleep = false;
#ifdef CONFIG_MODEM_HL7812_EDRX
   int  edrx_act_type;
   char set_edrx_msg[sizeof("AT+CEDRXS=2,4,\"0000\"")];
#endif
#if CONFIG_MODEM_HL7812_CONFIGURE_BANDS
   uint16_t bands_top    = 0;
   uint32_t bands_middle = 0, bands_bottom = 0;
   char     new_bands[MDM_BAND_BITMAP_STR_LENGTH_MAX + 1];
#endif
#if CONFIG_MODEM_HL7812_PSM
   char const TURN_ON_PSM[] = "AT+CPSMS=1,,,\"" CONFIG_MODEM_HL7812_PSM_PERIODIC_TAU
                              "\",\"" CONFIG_MODEM_HL7812_PSM_ACTIVE_TIME "\"";
#endif
...
reboot:
   ...
   if(!ictx.mdm_startup_reporting_on)
   {
       ...
       * Its value is saved in non-volatile memory on the HL7812.
       */
   ...
   /* If CONFIG_MODEM_HL7812_RAT_M1 or CONFIG_MODEM_HL7812_RAT_NB1, then
    ...
    */
#ifndef CONFIG_MODEM_HL7812_RAT_NO_CHANGE
   if(!ictx.configured)
   {
   #if CONFIG_MODEM_HL7812_RAT_M1
   ...
   #elif CONFIG_MODEM_HL7812_RAT_NB1
    ...
    */
#ifdef CONFIG_MODEM_HL7812_BOOT_IN_AIRPLANE_MODE
   SEND_AT_CMD_EXPECT_OK("AT+CFUN=4,0");
#endif
   ...
   /* Configure LTE bands */
#if CONFIG_MODEM_HL7812_CONFIGURE_BANDS
   #if CONFIG_MODEM_HL7812_BAND_1
   bands_bottom |= 1 << 0;
   #endif
   #if CONFIG_MODEM_HL7812_BAND_2
   bands_bottom |= 1 << 1;
   #endif
   #if CONFIG_MODEM_HL7812_BAND_3
   bands_bottom |= 1 << 2;
   #endif
   #if CONFIG_MODEM_HL7812_BAND_4
   bands_bottom |= 1 << 3;
   #endif
   #if CONFIG_MODEM_HL7812_BAND_5
   bands_bottom |= 1 << 4;
   #endif
   #if CONFIG_MODEM_HL7812_BAND_8
   bands_bottom |= 1 << 7;
   #endif
   #if CONFIG_MODEM_HL7812_BAND_9
   bands_bottom |= 1 << 8;
   #endif
   #if CONFIG_MODEM_HL7812_BAND_10
   bands_bottom |= 1 << 9;
   #endif
   #if CONFIG_MODEM_HL7812_BAND_12
   bands_bottom |= 1 << 11;
   #endif
   #if CONFIG_MODEM_HL7812_BAND_13
   bands_bottom |= 1 << 12;
   #endif
   #if CONFIG_MODEM_HL7812_BAND_14
   bands_bottom |= 1 << 13;
   #endif
   #if CONFIG_MODEM_HL7812_BAND_17
   bands_bottom |= 1 << 16;
   #endif
   #if CONFIG_MODEM_HL7812_BAND_18
   bands_bottom |= 1 << 17;
   #endif
   #if CONFIG_MODEM_HL7812_BAND_19
   bands_bottom |= 1 << 18;
   #endif
   #if CONFIG_MODEM_HL7812_BAND_20
   bands_bottom |= 1 << 19;
   #endif
   #if CONFIG_MODEM_HL7812_BAND_25
   bands_bottom |= 1 << 24;
   #endif
   #if CONFIG_MODEM_HL7812_BAND_26
   bands_bottom |= 1 << 25;
   #endif
   #if CONFIG_MODEM_HL7812_BAND_27
   bands_bottom |= 1 << 26;
   #endif
   #if CONFIG_MODEM_HL7812_BAND_28
   bands_bottom |= 1 << 27;
   #endif
   #if CONFIG_MODEM_HL7812_BAND_66
   bands_top |= 1 << 1;
   #endif
   ...
   ictx.low_power_mode = HL7812_LPM_NONE;
#ifdef CONFIG_MODEM_HL7812_LOW_POWER_MODE
   ...
   #if CONFIG_MODEM_HL7812_PSM
   ictx.low_power_mode = HL7812_LPM_PSM;
   /* Turn off eDRX */
   SEND_AT_CMD_EXPECT_OK("AT+CEDRXS=0");

   SEND_AT_CMD_EXPECT_OK(TURN_ON_PSM);
   #elif CONFIG_MODEM_HL7812_EDRX
   ictx.low_power_mode = HL7812_LPM_EDRX;
   /* Turn off PSM */
   ...
   snprintk(set_edrx_msg,
            sizeof(set_edrx_msg),
            "AT+CEDRXS=1,%d,\"%s\"",
            edrx_act_type,
            CONFIG_MODEM_HL7812_EDRX_VALUE);
   ...
   if(ictx.mdm_startup_state != HL7812_STARTUP_STATE_SIM_NOT_PRESENT)
   {
      /* query SIM ICCID */
   ...
   if(strcmp(ictx.mdm_pdp_addr_fam, MODEM_HL7812_ADDRESS_FAMILY))
   {
...
#if CONFIG_MODEM_HL7812_SET_APN_NAME_ON_STARTUP
   if(!ictx.configured)
   {
      if(strncmp(ictx.mdm_apn.value, CONFIG_MODEM_HL7812_APN_NAME, MDM_HL7812_APN_MAX_STRLEN) != 0)
      {
         ret = write_apn(CONFIG_MODEM_HL7812_APN_NAME);
         if(ret < 0)
         {
            goto error;
         }
...
#ifdef CONFIG_MODEM_HL7812_BOOT_DELAY
   if(!ictx.initialized)
   {
      if(ictx.iface != NULL)
      {
         hl7812_build_mac(&ictx);
         net_if_set_link_addr(ictx.iface, ictx.mac_addr, sizeof(ictx.mac_addr), NET_LINK_ETHERNET);
...
error:
   ...
   set_network_state(HL7812_UNABLE_TO_CONFIGURE);
...
static int write_apn(char *access_point_name)
{
   char cmd_string[MDM_HL7812_APN_CMD_MAX_SIZE];

   /* PDP Context */
   memset(cmd_string, 0, MDM_HL7812_APN_CMD_MAX_SIZE);
   if(strcmp(MODEM_HL7812_ADDRESS_FAMILY, ADDRESS_FAMILY_IPV4))
   {
      strncat(cmd_string,
              "AT+CGDCONT=1,\"" MODEM_HL7812_ADDRESS_FAMILY "\",\"",
              MDM_HL7812_APN_CMD_MAX_STRLEN);
   }
   else
   {
      strncat(cmd_string, "AT+CGDCONT=1,\"IP\",\"", MDM_HL7812_APN_CMD_MAX_STRLEN);
   }
   strncat(cmd_string, access_point_name, MDM_HL7812_APN_CMD_MAX_STRLEN);
   strncat(cmd_string, "\"", MDM_HL7812_APN_CMD_MAX_STRLEN);
   return send_at_cmd(NULL, cmd_string, MDM_CMD_SEND_TIMEOUT, 0, false);
}
...
static int hl7812_power_off(void)
{
   int ret = 0;
...
void mdm_hl7812_register_event_callback(struct mdm_hl7812_callback_agent *agent)
{
   ...
   sys_slist_append(&hl7812_event_callback_list, &agent->node);
   k_mutex_unlock(&cb_lock);
}
...
{
   k_mutex_lock(&cb_lock, K_FOREVER);
   (void)sys_slist_find_and_remove(&hl7812_event_callback_list, &agent->node);
   k_mutex_unlock(&cb_lock);
}

/*** OFFLOAD FUNCTIONS ***/

static int connect_TCP_socket(struct hl7812_socket *sock)
{
   int  ret;
...
static int configure_TCP_socket(struct hl7812_socket *sock)
{
   int  ret;
   ...
   snprintk(cmd_cfg,
            sizeof(cmd_cfg),
            "AT+KTCPCFG=%d,%d,\"%s\",%u,,,,%d,,%d",
            1,
            0,
            hl7812_sprint_ip_addr(&sock->dst),
            dst_port,
            af,
            restore_on_boot);
...
static int configure_UDP_socket(struct hl7812_socket *sock)
{
   int  ret = 0;
...
static int offload_bind(struct net_context *context, const struct sockaddr *addr, socklen_t addr_len)
{
   struct hl7812_socket *sock = NULL;
   ...
   sock = (struct hl7812_socket *)context->offload_context;
   if(!sock)
   {
...
static int
        offload_accept(struct net_context *context, net_tcp_accept_cb_t cb, int32_t timeout, void *user_data)
{
   /* NOT IMPLEMENTED */
   return -ENOTSUP;
}
...
static int offload_send(struct net_pkt *pkt, net_context_send_cb_t cb, int32_t timeout, void *user_data)
{
   struct net_context *context = net_pkt_context(pkt);
   socklen_t           addr_len;
...
static int
        offload_recv(struct net_context *context, net_context_recv_cb_t cb, int32_t timeout, void *user_data)
{
   struct hl7812_socket *sock;
...
static void hl7812_build_mac(struct hl7812_iface_ctx *ictx)
{
   ictx->mac_addr[0] = ictx->mdm_imei[MDM_HL7812_IMEI_STRLEN - 6];
   ictx->mac_addr[1] = ictx->mdm_imei[MDM_HL7812_IMEI_STRLEN - 5];
   ictx->mac_addr[2] = ictx->mdm_imei[MDM_HL7812_IMEI_STRLEN - 4];
   ictx->mac_addr[3] = ictx->mdm_imei[MDM_HL7812_IMEI_STRLEN - 3];
   ictx->mac_addr[4] = ictx->mdm_imei[MDM_HL7812_IMEI_STRLEN - 2];
   ictx->mac_addr[5] = ictx->mdm_imei[MDM_HL7812_IMEI_STRLEN - 1];
}
...
int32_t mdm_hl7812_update_fw(char *file_path)
{
   ...
   /* HL7812 will stay locked for the duration of the FW update */
   hl7812_lock();
   ...
   LOG_INF("Initiate FW update, total packets: %zd", ((file_info.size / XMODEM_DATA_SIZE) + 1));
   set_fota_state(HL7812_FOTA_START);
...
static int hl7812_init(const struct device *dev)
{
   int                        i, ret = 0;
   ...
   k_work_queue_start(&hl7812_workq,
                      hl7812_workq_stack,
                      K_THREAD_STACK_SIZEOF(hl7812_workq_stack),
                      WORKQ_PRIORITY,
                      &cfg);

   /* init work tasks */
   k_work_init_delayable(&ictx.rssi_query_work, hl7812_rssi_query_work);
   ...
   {
      if(!device_is_ready(hl7812_cfg.gpio[i].port))
      {
         LOG_ERR("gpio port (%s) not ready!", hl7812_cfg.gpio[i].port->name);
         return -ENODEV;
      }
   }

   ret = gpio_pin_configure_dt(&hl7812_cfg.gpio[MDM_RESET], GPIO_OUTPUT);
   if(ret)
   {
      LOG_ERR("Error configuring IO MDM_RESET %d err: %d!", hl7812_cfg.gpio[MDM_RESET].pin, ret);
      return ret;
   }

   ret = gpio_pin_configure_dt(&hl7812_cfg.gpio[MDM_WAKE], GPIO_OUTPUT);
   if(ret)
   {
      LOG_ERR("Error configuring IO MDM_WAKE %d err: %d!", hl7812_cfg.gpio[MDM_WAKE].pin, ret);
      return ret;
   }

   ret = gpio_pin_configure_dt(&hl7812_cfg.gpio[MDM_PWR_ON], GPIO_OUTPUT);
   if(ret)
   {
      LOG_ERR("Error configuring IO MDM_PWR_ON %d err: %d!", hl7812_cfg.gpio[MDM_PWR_ON].pin, ret);
      return ret;
   }

   ret = gpio_pin_configure_dt(&hl7812_cfg.gpio[MDM_FAST_SHUTD], GPIO_OUTPUT);
   if(ret)
   {
      LOG_ERR("Error configuring IO MDM_FAST_SHUTD %d err: %d!", hl7812_cfg.gpio[MDM_FAST_SHUTD].pin, ret);
      return ret;
   }

   ret = gpio_pin_configure_dt(&hl7812_cfg.gpio[MDM_VGPIO], GPIO_INPUT);
   if(ret)
   {
      LOG_ERR("Error configuring IO MDM_VGPIO %d err: %d!", hl7812_cfg.gpio[MDM_VGPIO].pin, ret);
      return ret;
   }

   // ret = gpio_pin_configure_dt(&hl7812_cfg.gpio[MDM_UART_DSR], GPIO_INPUT);
   // if(ret)
   // {
   //    LOG_ERR("Error configuring IO MDM_UART_DSR %d err: %d!", hl7812_cfg.gpio[MDM_UART_DSR].pin, ret);
   //    return ret;
   // }

   // ret = gpio_pin_configure_dt(&hl7812_cfg.gpio[MDM_UART_CTS], GPIO_INPUT);
   // if(ret)
   // {
   //    LOG_ERR("Error configuring IO MDM_UART_CTS %d err: %d!", hl7812_cfg.gpio[MDM_UART_CTS].pin, ret);
   //    return ret;
   // }

   ret = gpio_pin_configure_dt(&hl7812_cfg.gpio[MDM_GPIO6], GPIO_INPUT);
   if(ret)
   {
      LOG_ERR("Error configuring IO MDM_GPIO6 %d err: %d!", hl7812_cfg.gpio[MDM_GPIO6].pin, ret);
      return ret;
   }
    ...
    * This allows HL7812 VGPIO to be high, which is good because the UART
   ...
   /* VGPIO */
   gpio_init_callback(&ictx.mdm_vgpio_cb, mdm_vgpio_callback_isr, BIT(hl7812_cfg.gpio[MDM_VGPIO].pin));
   ret = gpio_add_callback(hl7812_cfg.gpio[MDM_VGPIO].port, &ictx.mdm_vgpio_cb);
   if(ret)
   {
      LOG_ERR("Cannot setup vgpio callback! (%d)", ret);
      return ret;
   }
   ret = gpio_pin_interrupt_configure_dt(&hl7812_cfg.gpio[MDM_VGPIO], GPIO_INT_EDGE_BOTH);
   if(ret)
   {
   ...
   // /* UART DSR */
   // gpio_init_callback(&ictx.mdm_uart_dsr_cb,
   //                    mdm_uart_dsr_callback_isr,
   //                    BIT(hl7812_cfg.gpio[MDM_UART_DSR].pin));
   // ret = gpio_add_callback(hl7812_cfg.gpio[MDM_UART_DSR].port, &ictx.mdm_uart_dsr_cb);
   // if(ret)
   // {
   //    LOG_ERR("Cannot setup uart dsr callback! (%d)", ret);
   //    return ret;
   // }
   // ret = gpio_pin_interrupt_configure_dt(&hl7812_cfg.gpio[MDM_UART_DSR], GPIO_INT_EDGE_BOTH);
   // if(ret)
   // {
   ...
   // /* GPIO6 */
   // gpio_init_callback(&ictx.mdm_gpio6_cb, mdm_gpio6_callback_isr, BIT(hl7812_cfg.gpio[MDM_GPIO6].pin));
   // ret = gpio_add_callback(hl7812_cfg.gpio[MDM_GPIO6].port, &ictx.mdm_gpio6_cb);
   // if(ret)
   // {
   //    LOG_ERR("Cannot setup gpio6 callback! (%d)", ret);
   //    return ret;
   // }
   // ret = gpio_pin_interrupt_configure_dt(&hl7812_cfg.gpio[MDM_GPIO6], GPIO_INT_EDGE_BOTH);
   // if(ret)
   // {
   ...
   // BIT(hl7812_cfg.gpio[MDM_UART_CTS].pin)); ret = gpio_add_callback(hl7812_cfg.gpio[MDM_UART_CTS].port,
   // &ictx.mdm_uart_cts_cb); if(ret)
   // {
   //    LOG_ERR("Cannot setup uart cts callback! (%d)", ret);
   //    return ret;
   // }
   // ret = gpio_pin_interrupt_configure_dt(&hl7812_cfg.gpio[MDM_UART_CTS], GPIO_INT_EDGE_BOTH);
   // if(ret)
   // {
   ...
   ictx.mdm_ctx.data_imsi  = ictx.mdm_imsi;
   ictx.mdm_ctx.data_iccid = ictx.mdm_iccid;
#endif
   ictx.mdm_ctx.data_rssi = &ictx.mdm_rssi;

   ret = mdm_receiver_register(&ictx.mdm_ctx, MDM_UART_DEV, mdm_recv_buf, sizeof(mdm_recv_buf));
   if(ret < 0)
   {
   ...
   /* start RX thread */
   k_thread_name_set(k_thread_create(&hl7812_rx_thread,
                                     hl7812_rx_stack,
                                     K_THREAD_STACK_SIZEOF(hl7812_rx_stack),
                                     (k_thread_entry_t)hl7812_rx,
                                     NULL,
                                     NULL,
                                     NULL,
                                     RX_THREAD_PRIORITY,
...
static void offload_iface_init(struct net_if *iface)
{
   ...
   struct hl7812_iface_ctx *ctx = dev->data;

   iface->if_dev->offload       = &offload_funcs;
   ctx->iface                   = iface;

   if(!IS_ENABLED(CONFIG_MODEM_HL7812_BOOT_DELAY))
   {
      hl7812_build_mac(&ictx);
...
NET_DEVICE_DT_INST_OFFLOAD_DEFINE(0,
                                  ...
                                  &ictx,
                                  &hl7812_cfg,
                                  CONFIG_MODEM_HL7812_INIT_PRIORITY,
                                  &api_funcs,
                                  MDM_MTU);
